...
class Launcher:
    ...
    def __init__(
        self, description: str, long_text: str = "", argv: Optional[List[str]] = None
    ):
        ...
        parser = argparse.ArgumentParser(
            description=f"{description} : {long_text}", epilog=epilog
        )
        ...
        if args.trial_config_repeat_count:
            self.global_config["trial_config_repeat_count"] = (
                args.trial_config_repeat_count
            )
        ...
        self.global_config = DictTemplater(self.global_config).expand_vars(
            use_os_env=True
        )
    ...
    def _init_tunable_values(
        ...
        if args_tunables is not None:
            for data_file in args_tunables:
                values = self._config_loader.load_config(
                    data_file, ConfigSchema.TUNABLE_VALUES
                )
    ...
    def _load_optimizer(self, args_optimizer: Optional[str]) -> Optimizer:
        ...
        class_config = self._config_loader.load_config(
            args_optimizer, ConfigSchema.OPTIMIZER
        )
    ...
    def _load_storage(self, args_storage: Optional[str]) -> Storage:
        ...
        class_config = self._config_loader.load_config(
            args_storage, ConfigSchema.STORAGE
        )
        assert isinstance(class_config, Dict)
    ...
    def _load_scheduler(self, args_scheduler: Optional[str]) -> Scheduler:
        ...
        class_config = self._config_loader.load_config(
            args_scheduler, ConfigSchema.SCHEDULER
        )
        assert isinstance(class_config, Dict)
