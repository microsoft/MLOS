...
class Trial(Storage.Trial):
            ...
            try:
                if status.is_completed():
                    ...
                    if cur_status.rowcount not in {1, -1}:
                        ...
                        _LOG.warning(
                            "Trial %s :: cannot be updated to: %s", self, status
                        )
            except Exception:
                conn.rollback()
                raise
    ...
    def update_telemetry(
        ...
        timestamp = utcify_timestamp(timestamp, origin="local")
        metrics = [
            (utcify_timestamp(ts, origin="local"), key, val)
        ...
        for metric_ts, key, val in metrics:
            with self._engine.begin() as conn:
                try:
                    ...
                    _LOG.warning(
                        "Record already exists: %s :: %s", (metric_ts, key, val), ex
                    )

    def _update_status(
        self, conn: Connection, status: Status, timestamp: datetime
    ) -> None:
        """
