...
def _tunable_to_configspace(
        ...
        return ConfigurationSpace(
            {
                tunable.name: CategoricalHyperparameter(
                    ...
                    weights=(
                        _normalize_weights(tunable.weights) if tunable.weights else None
                    ),
    ...
    if tunable.weights and tunable.range_weight is not None:
        ...
        switch_weights = _normalize_weights(
            [sum(tunable.weights), tunable.range_weight]
        )
    ...
    conf_space = ConfigurationSpace(
        {
            ...
            special_name: CategoricalHyperparameter(
                ...
                default_value=(
                    tunable.default if tunable.default in tunable.special else None
                ),
                meta=meta,
            ),
...
def configspace_data_to_tunable_values(data: dict) -> Dict[str, TunableValue]:
    ...
    specials = [
        ...
        for k in data.keys()
        if special_param_name_is_temp(k)
    ]
    for k in specials:
