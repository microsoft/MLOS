...
class GridSearchOptimizer(TrackBestOptimizer):
    ...
    def pending_configs(self) -> Iterable[Dict[str, TunableValue]]:
        ...
        # See NOTEs above.
        return (
            dict(zip(self._config_keys, config))
            for config in self._pending_configs.keys()
        )
    ...
    def suggested_configs(self) -> Iterable[Dict[str, TunableValue]]:
        ...
        # See NOTEs above.
        return (
            dict(zip(self._config_keys, config)) for config in self._suggested_configs
        )
        ...
        try:
            config = dict(
                ConfigSpace.Configuration(
                    self.config_space, values=tunables.get_param_values()
                )
            )
            self._suggested_configs.remove(tuple(config.values()))
        except KeyError:
            _LOG.warning(
                "Attempted to remove missing config (previously registered?) from suggested set: %s",
                tunables,
            )
