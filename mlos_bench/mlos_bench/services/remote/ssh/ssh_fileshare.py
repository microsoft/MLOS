...
class SshFileShareService(FileShareService, SshService):
    ...
    async def _start_file_copy(
        ...
        return await scp(
            srcpaths=srcpaths, dstpath=dstpath, recurse=recursive, preserve=True
        )
    ...
    def download(
        ...
        file_copy_future = self._run_coroutine(
            self._start_file_copy(
                params, CopyMode.DOWNLOAD, local_path, remote_path, recursive
            )
        )
        try:
            ...
            if isinstance(ex, SFTPNoSuchFile) or (
                ...
                raise FileNotFoundError(
                    f"File {remote_path} does not exist on {params}"
                ) from ex
            raise ex
    ...
    def upload(
        ...
        file_copy_future = self._run_coroutine(
            self._start_file_copy(
                params, CopyMode.UPLOAD, local_path, remote_path, recursive
            )
        )
        try:
            file_copy_future.result()
            ...
            _LOG.error(
                "Failed to upload %s to %s on %s: %s",
                local_path,
                remote_path,
                params,
                ex,
