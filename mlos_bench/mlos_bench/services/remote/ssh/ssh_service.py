...
class SshClientCache:
    ...
    async def get_client_connection(
        ...
        """
        _LOG.debug(
            "%s: get_client_connection: %s", current_thread().name, connect_params
        )
...
class SshService(Service, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        self._request_timeout = self.config.get(
            "ssh_request_timeout", self._REQUEST_TIMEOUT
        )
        ...
        if "ssh_known_hosts_file" in self.config:
            self._connect_params["known_hosts"] = self.config.get(
                "ssh_known_hosts_file", None
            )
            if isinstance(self._connect_params["known_hosts"], str):
                known_hosts_file = os.path.expanduser(
                    self._connect_params["known_hosts"]
                )
                if not os.path.exists(known_hosts_file):
                    raise ValueError(
                        f"ssh_known_hosts_file {known_hosts_file} does not exist"
                    )
        ...
        if "ssh_keepalive_interval" in self.config:
            ...
            self._connect_params["keepalive_interval"] = nullable(
                int, keepalive_internal
            )
    ...
    async def _get_client_connection(
        self, params: dict
    ) -> Tuple[SSHClientConnection, SshClient]:
        """
        ...
        assert self._in_context
        return (
            await SshService._EVENT_LOOP_THREAD_SSH_CLIENT_CACHE.get_client_connection(
