...
class ConfigPersistenceService(Service, SupportsConfigLoading):
    ...
    def resolve_path(
        self, file_path: str, extra_paths: Optional[Iterable[str]] = None
    ) -> str:
        """
    ...
    def prepare_class_load(
        ...
        for key in set(class_config).intersection(
            config.get("resolve_config_property_paths", [])
        ):
            if isinstance(class_config[key], str):
                ...
                class_config[key] = [
                    self.resolve_path(path) for path in class_config[key]
                ]
            else:
    ...
    def build_environment(
        ...
        (env_class, env_config) = self.prepare_class_load(
            config, global_config, parent_args
        )
    ...
    def load_environment(
        ...
        return self.build_environment(
            config, tunables, global_config, parent_args, service
        )
    ...
    def load_services(
        ...
        """
        _LOG.info(
            "Load services: %s parent: %s", json_file_names, parent.__class__.__name__
        )
        service = Service({}, global_config, parent)
        for fname in json_file_names:
            config = self.load_config(fname, ConfigSchema.SERVICE)
            service.register(
                self.build_service(config, global_config, service).export()
            )
        return service
