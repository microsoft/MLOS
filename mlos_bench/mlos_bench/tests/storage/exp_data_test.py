...
def test_exp_data_tunable_config_trial_group_id_in_results_df(
...
def test_exp_data_tunable_config_trial_groups(exp_data: ExperimentData) -> None:
    ...
    assert list(exp_data.tunable_config_trial_groups.keys()) == list(
    ...
    assert [
        ...
        range(1, CONFIG_COUNT * CONFIG_TRIAL_REPEAT_COUNT, CONFIG_TRIAL_REPEAT_COUNT)
    )
...
def test_exp_data_tunable_configs(exp_data: ExperimentData) -> None:
    ...
    # Which should match the objects.
    assert [
        config.tunable_config_id for config in exp_data.tunable_configs.values()
    ] == list(range(1, CONFIG_COUNT + 1))
