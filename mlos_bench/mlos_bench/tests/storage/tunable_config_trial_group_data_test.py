...
def test_tunable_config_trial_group_data(exp_data: ExperimentData) -> None:
    ...
    assert (
        tunable_config_trial_group.experiment_id
    ...
    assert (
        ...
        == next(
            iter(tunable_config_trial_group.trials.values())
        ).tunable_config_trial_group
    )
...
def test_tunable_config_trial_group_results_df(
    ...
    assert (
        len(results_df[(results_df["tunable_config_id"] == tunable_config_id)])
        == expected_count
    )
    ...
    assert (
        len(
            results_df[
                (results_df["tunable_config_trial_group_id"] == expected_group_id)
            ]
        )
        == expected_count
    )
    ...
    assert (
        len(results_df[ExperimentData.RESULT_COLUMN_PREFIX + obj_target])
        == expected_count
    )
...
def test_tunable_config_trial_group_trials(exp_data: ExperimentData) -> None:
    ...
    assert all(
        trial.tunable_config_trial_group.tunable_config_trial_group_id
        == expected_group_id
    ...
    assert (
        exp_data.trials[expected_group_id]
        == tunable_config_trial_group.trials[expected_group_id]
    )
