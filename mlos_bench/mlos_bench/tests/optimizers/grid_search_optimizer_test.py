...
def grid_search_tunables_grid(
    ...
    return list(
        dict(zip(tunable_names, combo))
        for combo in itertools.product(*tunables_params_values)
    )
...
def test_grid_search_grid(
    ...
    expected_grid_size = math.prod(
        tunable.cardinality for tunable, _group in grid_search_tunables
    )
    ...
    assert all(
        config in grid_search_tunables_grid
        for config in grid_search_opt_pending_configs
    )
    assert all(
        config in grid_search_opt_pending_configs
        for config in grid_search_tunables_grid
    )
...
def test_grid_search(
    ...
    assert all(
        config in grid_search_tunables_grid
        for config in grid_search_opt.pending_configs
    )
    assert all(
        config in list(grid_search_opt.pending_configs)
        for config in grid_search_tunables_grid
    )
...
def test_grid_search_async_order(grid_search_opt: GridSearchOptimizer) -> None:
    ...
    assert all(
        suggestion.get_param_values() not in suggested_shuffled
        for suggestion in suggested
    )
