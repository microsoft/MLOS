...
def test_launcher_args_parse_1(config_paths: List[str]) -> None:
    ...
    assert path_join(
        launcher.global_config["pathVarWithEnvVarRef"], abs_path=True
    ...
    env_config = launcher.config_loader.load_config(
        env_conf_path, ConfigSchema.ENVIRONMENT
    )
    ...
    assert (
        launcher.scheduler._trial_config_repeat_count == 3
...
def test_launcher_args_parse_2(config_paths: List[str]) -> None:
    ...
    env_config = launcher.config_loader.load_config(
        env_config_file, ConfigSchema.ENVIRONMENT
    )
    assert check_class_name(launcher.environment, env_config["class"])
    ...
    opt_config = launcher.config_loader.load_config(
        opt_config_file, ConfigSchema.OPTIMIZER
    )
    globals_file_config = launcher.config_loader.load_config(
        globals_file, ConfigSchema.GLOBALS
    )
    ...
    assert isinstance(launcher.scheduler, SyncScheduler)
    assert (
        launcher.scheduler._trial_config_repeat_count == 5
    )  # pylint: disable=protected-access
