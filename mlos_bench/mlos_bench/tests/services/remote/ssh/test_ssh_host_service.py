...
def test_ssh_service_remote_exec(
    ...
    with ssh_host_service:
        ...
        connection_client = (
            ssh_host_service._EVENT_LOOP_THREAD_SSH_CLIENT_CACHE._cache.get(
                connection_id
            )
        )
        ...
        connection, client = (
            ssh_host_service._EVENT_LOOP_THREAD_SSH_CLIENT_CACHE._cache[connection_id]
        )
...
def check_ssh_service_reboot(
    ...
    with ssh_host_service:
        reboot_test_srv_ssh_svc_conf = reboot_test_server.to_ssh_service_config(
            uncached=True
        )
        (status, results_info) = ssh_host_service.remote_exec(
            script=[
        ...
        (status, reboot_results_info) = ssh_host_service.wait_os_operation(
            reboot_results_info
        )
        ...
        for _ in range(0, 3):
            ...
            try:
                ...
                reboot_test_srv_ssh_svc_conf_new = (
                    reboot_test_server.to_ssh_service_config(uncached=True)
                )
                if (
