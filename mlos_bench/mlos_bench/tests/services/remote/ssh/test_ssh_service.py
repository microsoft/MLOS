...
def test_ssh_service_test_infra(
    ssh_test_server_info: SshTestServerInfo, server_name: str
) -> None:
...
def test_ssh_service_context_handler() -> None:
    ...
    with ssh_host_service:
        ...
        ssh_fileshare_service = SshFileShareService(
            config={}, global_config={}, parent=None
        )
        ...
        with pytest.raises(
            ...
            future = ssh_fileshare_service._run_coroutine(
                asyncio.sleep(0.1, result="foo")
            )
            raise ValueError(
                f"Future should not have been available to wait on {future.result()}"
            )
