...
def test_wait_host_deployment_retry(
    ...
    mock_getconn.return_value.getresponse.side_effect = [
        make_httplib_json_response(
            200, {"properties": {"provisioningState": "Running"}}
        ),
...
def test_azure_vm_service_recursive_template_params(
...
def test_azure_vm_service_custom_data(azure_auth_service: AzureAuthService) -> None:
    ...
    with pytest.raises(ValueError):
        ...
        AzureVMService(
            config_with_custom_data, global_config, parent=azure_auth_service
        )
    azure_vm_service = AzureVMService(config, global_config, parent=azure_auth_service)
...
def test_get_remote_exec_results(
    ...
    setattr(
        ...
        "wait_host_operation",
        mock_wait_host_operation,
    )

    status, cmd_output = azure_vm_service_remote_exec_only.get_remote_exec_results(
        params
    )
