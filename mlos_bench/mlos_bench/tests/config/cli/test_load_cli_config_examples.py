...
def test_load_cli_config_examples(
    ...
    if config_paths := config.get("config_path"):
        ...
        for path in config_paths:
            config_loader_service._config_path.insert(
                0, path
    ...
    for arg in config:
        ...
        if arg == "globals":
            for path in config[arg]:
                sub_config = config_loader_service.load_config(
                    path, ConfigSchema.GLOBALS
                )
                assert isinstance(sub_config, dict)
        elif arg == "environment":
            sub_config = config_loader_service.load_config(
                config[arg], ConfigSchema.ENVIRONMENT
            )
            assert isinstance(sub_config, dict)
        elif arg == "optimizer":
            sub_config = config_loader_service.load_config(
                config[arg], ConfigSchema.OPTIMIZER
            )
            assert isinstance(sub_config, dict)
        elif arg == "storage":
            sub_config = config_loader_service.load_config(
                config[arg], ConfigSchema.STORAGE
            )
            assert isinstance(sub_config, dict)
        elif arg == "tunable_values":
            for path in config[arg]:
                sub_config = config_loader_service.load_config(
                    path, ConfigSchema.TUNABLE_VALUES
                )
                assert isinstance(sub_config, dict)
        else:
...
def test_load_cli_config_examples_via_launcher(
    ...
    launcher = Launcher(
        description=__name__, long_text=config_path, argv=cli_args.split()
    )
    assert launcher
    ...
    assert (
        ...
        in launcher.config_loader.config_paths
    )
    if config_paths := config.get("config_path"):
        assert isinstance(config_paths, list)
        for path in config_paths:
            ...
            assert any(
                config_path.endswith(path)
                for config_path in launcher.config_loader.config_paths
