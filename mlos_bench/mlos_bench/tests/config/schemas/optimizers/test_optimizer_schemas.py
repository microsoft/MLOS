...
@pytest.mark.parametrize(
    "mlos_bench_optimizer_type", expected_mlos_bench_optimizer_class_names
)
...
def test_case_coverage_mlos_core_optimizer_type(
    ...
    """
    optimizer_name = (
        None if mlos_core_optimizer_type is None else mlos_core_optimizer_type.name
    )
...
# @pytest.mark.parametrize("test_case_subtype", sorted(TEST_CASES.by_subtype))
@pytest.mark.parametrize(
    "mlos_core_space_adapter_type", expected_mlos_core_space_adapter_types
)
def test_case_coverage_mlos_core_space_adapter_type(
    ...
    space_adapter_name = (
        None
        if mlos_core_space_adapter_type is None
        else mlos_core_space_adapter_type.name
    )
    for test_case in TEST_CASES.by_type[test_case_type].values():
        if (
            try_resolve_class_name(test_case.config.get("class"))
            ...
            if test_case.config.get("config"):
                space_adapter_type = test_case.config["config"].get(
                    "space_adapter_type", None
                )
...
def test_optimizer_configs_against_schema(test_case_name: str) -> None:
    ...
    """
    check_test_case_against_schema(
        TEST_CASES.by_path[test_case_name], ConfigSchema.OPTIMIZER
    )
    check_test_case_against_schema(
