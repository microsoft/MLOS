...
class LocalEnv(ScriptEnv):
    ...
    def __init__(
        ...
        *,
        ...
        self._read_telemetry_file: Optional[str] = self.config.get(
            "read_telemetry_file"
        )
    ...
    def setup(
        ...
        if self._script_setup:
            (return_code, _output) = self._local_exec(
                self._script_setup, self._temp_dir
            )
    ...
    def run(self) -> Tuple[Status, datetime, Optional[Dict[str, TunableValue]]]:
        ...
        if list(data.columns) == ["metric", "value"]:
            ...
            data = pandas.DataFrame(
                [data.value.to_list()], columns=data.metric.to_list()
            )
    ...
    def status(self) -> Tuple[Status, datetime, List[Tuple[datetime, str, Any]]]:
        ...
        try:
            ...
            if len(data.columns) != len(expected_col_names):
                raise ValueError(
                    f"Telemetry data must have columns {expected_col_names}"
                )
            ...
            _LOG.warning(
                "Telemetry CSV file not found: %s :: %s", self._read_telemetry_file, ex
            )
            return (status, timestamp, [])
        ...
        return (
            ...
            [
                ...
                for (ts, metric, value) in data.to_records(
                    index=False, column_dtypes=col_dtypes
                )
            ],
        )
    ...
    def _local_exec(
        self, script: Iterable[str], cwd: Optional[str] = None
    ) -> Tuple[int, dict]:
        """
        ...
        if return_code != 0:
            _LOG.warning(
                "ERROR: Local script returns code %d stderr:\n%s", return_code, stderr
            )
