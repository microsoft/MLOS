...
def _main(input_file: str, output_file: str) -> None:
    ...
    df_long = pd.concat(
        [
            df_long,
            pd.DataFrame(
                {"metric": ["score"], "value": [df_long.value[df_long.index.max()]]}
            ),
        ]
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Post-process Redis benchmark results."
    )
    parser.add_argument(
        "input", help="Redis benchmark results (downloaded from a remote VM)."
    )
    parser.add_argument(
        "output",
        help="Converted Redis benchmark data"
