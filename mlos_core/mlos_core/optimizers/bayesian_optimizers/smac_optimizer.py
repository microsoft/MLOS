...
class SmacOptimizer(BaseBayesianOptimizer):
    ...
    def _register(
        ...
        *,
        ...
        for config, (_i, score) in zip(
            ...
            value = TrialValue(
                cost=list(score.astype(float)), time=0.0, status=StatusType.SUCCESS
            )
    ...
    def surrogate_predict(
        ...
        if self._space_adapter and not isinstance(self._space_adapter, IdentityAdapter):
            raise NotImplementedError(
                "Space adapter not supported for surrogate_predict."
            )
    ...
    def cleanup(self) -> None:
        if (
            hasattr(self, "_temp_output_directory")
            and self._temp_output_directory is not None
        ):
