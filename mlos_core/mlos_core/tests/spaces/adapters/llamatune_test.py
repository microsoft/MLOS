    ...
    (
        [
            ...
            for param_space_kwargs in (
                {
                    "n_continuous_params": int(
                        num_target_space_dims * num_orig_space_factor
                    )
                },
                {
                    "n_integer_params": int(
                        num_target_space_dims * num_orig_space_factor
                    )
                },
                {
                    "n_categorical_params": int(
                        num_target_space_dims * num_orig_space_factor
                    )
                },
                ...
                {
                    "n_continuous_params": int(
                        num_target_space_dims * num_orig_space_factor / 3
                    ),
                    "n_integer_params": int(
                        num_target_space_dims * num_orig_space_factor / 3
                    ),
...
def test_num_low_dims(
    ...
    for (
        sampled_config
    ) in sampled_configs:  # pylint: disable=not-an-iterable # (false positive)
        ...
        orig_config = CS.Configuration(
            input_space, values=orig_config_df.iloc[0].to_dict()
        )
...
def test_special_parameter_values_validation() -> None:
    ...
    input_space.add_hyperparameter(
        CS.CategoricalHyperparameter(
            name="str", choices=[f"choice_{idx}" for idx in range(5)]
        )
    )
    input_space.add_hyperparameter(
    ...
    invalid_special_param_values_dicts: List[Dict[str, Any]] = [
        ...
        {
            "int": (0.4, 0)
        },  # (biasing %, special value) instead of (special value, biasing %)
...
def gen_random_configs(
    adapter: LlamaTuneAdapter, num_configs: int
) -> Iterator[CS.Configuration]:
    for sampled_config in adapter.target_parameter_space.sample_configuration(
        size=num_configs
    ):
...
def test_special_parameter_values_biasing() -> None:  # pylint: disable=too-complex
    ...
    for spv_dict in special_param_value_dicts:
        ...
        assert (1 - eps) * int(
            num_configs * bias_percentage
        ) <= special_values_occurrences[0]
        assert (1 - eps) * int(
            num_configs * bias_percentage
        ) <= special_values_occurrences[1]
    ...
    assert (1 - eps) * int(num_configs * bias_percentage) <= special_values_instances[
        "int_1"
    ][0]
    assert (1 - eps) * int(
        num_configs * bias_percentage / 2
    ) <= special_values_instances["int_1"][1]
    assert (1 - eps) * int(
        num_configs * bias_percentage / 2
    ) <= special_values_instances["int_2"][2]
    assert (1 - eps) * int(
        num_configs * bias_percentage * 1.5
...
def test_max_unique_values_per_param() -> None:
    ...
    input_space.add_hyperparameter(
        CS.CategoricalHyperparameter(
            name="str_2", choices=[f"choice_{idx}" for idx in range(10)]
        )
    )
    ...
    for max_unique_values_per_param in (5, 25, 100):
        ...
        unique_values_dict: Dict[str, set] = {
            param: set() for param in list(input_space.keys())
        }
        for config in gen_random_configs(adapter, num_configs):
...
def test_llamatune_pipeline(
    ...
    for (
        ...
        for value, bias_percentage in tuples_list:
            assert (1 - eps) * int(
                num_configs * bias_percentage
            ) <= special_value_occurrences[param][value]
...
def test_deterministic_behavior_for_same_seed(
    ...
    assert generate_target_param_space_configs(
        42
    ) == generate_target_param_space_configs(42)
    assert generate_target_param_space_configs(
        1234
    ) != generate_target_param_space_configs(42)
