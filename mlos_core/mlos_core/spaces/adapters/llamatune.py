...
class LlamaTuneAdapter(BaseSpaceAdapter):  # pylint: disable=too-many-instance-attributes
    ...
    """
    Default number of dimensions in the low-dimensional search space, generated
    by HeSBO projection
    """
    ...
    def inverse_transform(self, configurations: pd.DataFrame) -> pd.DataFrame:
        ...
        for _, config in configurations.astype("O").iterrows():
            ...
            # NOTE: HeSBO is a non-linear projection method, and does not
            # inherently support inverse projection.
            # To (partly) support this operation, we keep track of the
            # suggested low-dim point(s) along with the respective high-dim
            # point; this way we can retrieve the low-dim point, from its
            # high-dim counterpart.
            if target_config is None:
                # Inherently it is not supported to register points, which were
                # not suggested by the optimizer.
    ...
    def _construct_low_dim_space(
        ...
        if max_unique_values_per_param is None:
            ...
            # Currently supported optimizers do not support defining a
            # discretized space (like ConfigSpace does using `q` kwarg).
            # Thus, to support space discretization, we define the
            # low-dimensional space using integer hyperparameters.
            ...
            # Initialize quantized values scaler: from [0,
            # max_unique_values_per_param] to (-1, 1) range
    ...
    def _transform(self, configuration: dict) -> dict:
        """
        Projects a low-dimensional point (configuration) to the
        high-dimensional original parameter space, and then biases the
        resulting parameter values towards their special value(s) (if any).
    ...
    def _special_param_value_scaler(
        ...
        """
        Biases the special value(s) of this parameter, by shifting the
        normalized `input_value` towards those.
        ...
        for param, value in special_param_values_dict.items():
            ...
            if isinstance(value, int):
                ...
                if all(isinstance(t, int) for t in value):
                    ...
                    # User specifies list of tuples; each tuple defines the
                    # special value and the biasing percentage
                    tuple_list = value
                else:
                    raise ValueError(
    ...
    def _try_generate_approx_inverse_mapping(self) -> None:
        """
        Tries to generate an approximate reverse mapping: i.e., from
        high-dimensional space to the low-dimensional one.
        Reverse mapping is generated using the pseudo-inverse matrix, of
        original HeSBO projection matrix.
        This mapping can be potentially used to register configurations that
        were *not* previously suggested by the optimizer.

        NOTE: This method is experimental, and there is currently no guarantee
