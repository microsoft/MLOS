...
def augment_results_df_with_config_trial_group_stats(
    ...
    *,
    ...
    def compute_zscore_for_group_agg(
        ...
        results_groups_perf_aggs = results_groups_perf.agg(
            agg
        ...
        stats_df.drop(
            columns=[result_col + ".var_" + agg for agg in ("mean", "stddev")],
            inplace=True,
        )

    augmented_results_df = results_df
    augmented_results_df["tunable_config_trial_group_size"] = results_groups[
        "trial_id"
    ].transform("count")
    for result_col in result_cols:
        ...
        stats_df = pandas.DataFrame()
        stats_df[result_col + ".mean"] = results_groups_perf.transform(
            "mean", numeric_only=True
        )
        stats_df[result_col + ".var"] = results_groups_perf.transform("var")
        stats_df[result_col + ".stddev"] = stats_df[result_col + ".var"].apply(
            lambda x: x**0.5
        )
...
def limit_top_n_configs(
    ...
    *,
    ...
    default_config_id = (
        results_df[trial_id_col].min()
        if exp_data is None
        else exp_data.default_tunable_config_id
    )
    ...
    default_config_results_df = results_df.loc[
        results_df[config_id_col] == default_config_id
    ]
...
def plot_optimizer_trends(
    ...
    *,
    ...
    for objective_column, ascending in obj_cols.items():
        ...
        if ascending:
            group_results_df[incumbent_column] = group_results_df[
                objective_column
            ].cummin()
        else:
            group_results_df[incumbent_column] = group_results_df[
...
def plot_top_n_configs(
    ...
    **kwargs: Any,
    ...
    (top_n_config_results_df, _groupby_columns, groupby_column) = (
        _add_groupby_desc_column(top_n_config_results_df)
    )
