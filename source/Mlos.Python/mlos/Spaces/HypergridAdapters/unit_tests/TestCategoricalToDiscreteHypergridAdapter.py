...
class TestCategoricalToDiscreteHypergridAdapter:
    ...
    def test_projecting_point_to_unit_continuous_hypergrid(self):
        ...
        for hypergrid, adapter in hypergrdis_and_adapters:
            for _ in range(1000):
                ...
                assert all(isinstance(dim_value, float) and 0 <= dim_value <= 1 for _, dim_value in projected_point)
                assert projected_point in adapter
                unprojected_point = adapter.unproject_point(projected_point)
                assert original_point == unprojected_point
                assert unprojected_point in hypergrid
    ...
    def test_projecting_dataframe_from_flat_to_unit_continuous_hypergrid(self):
        ...
        for column in projected_df.columns:
            assert projected_df[column].between(0, 1).all()
    ...
    def test_projecting_dataframe_from_categorical_to_discrete_simple_hypergrid(self):
        ...
        #
        assert id(original_df) != id(projected_df) # Make sure that a deep copy was made.
        ...
        for column in columns:
            ...
            assert pd.to_numeric(copied_df[column], errors='coerce').notnull().all()
        ...
        assert not pd.to_numeric(copied_original_df['categorical_mixed_types'], errors='coerce').notnull().all()


        unprojected_df = adapter.unproject_dataframe(projected_df, in_place=False)
        assert id(original_df) != id(unprojected_df)
        assert original_df.equals(unprojected_df)
