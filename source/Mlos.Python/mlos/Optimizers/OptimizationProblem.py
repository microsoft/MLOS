...
class OptimizationProblem:
    ...
    def to_protobuf(self):
        ...
        return OptimizerMonitoringService_pb2.OptimizationProblem(
            ParameterSpace=OptimizerMonitoringService_pb2.Hypergrid(HypergridJsonString=json.dumps(self.parameter_space, cls=HypergridJsonEncoder)),
            ObjectiveSpace=OptimizerMonitoringService_pb2.Hypergrid(HypergridJsonString=json.dumps(self.objective_space, cls=HypergridJsonEncoder)),
            Objectives=[OptimizerMonitoringService_pb2.Objective(Name=objective.name, Minimize=objective.minimize) for objective in self.objectives],
            ContextSpace=None if self.context_space is None
            else OptimizerMonitoringService_pb2.Hypergrid(HypergridJsonString=json.dumps(self.context_space, cls=HypergridJsonEncoder))
        )

    @classmethod
    def from_protobuf(cls, optimization_problem_pb2: OptimizerMonitoringService_pb2.OptimizationProblem):
